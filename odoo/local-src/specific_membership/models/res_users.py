...
class ResUsers(osv.Model):
    ...
    def send_account_confirmation_email(self, cr, uid, login, context=None):
        ...
        if not user_ids:
            user_ids = self.search(
                cr, uid, [
                    ('email', '=', login)], context=context)
        if len(user_ids) != 1:
            raise Exception(
                _('Reset password: invalid username or email'))
        return self.action_account_confirmation_email(
            cr,
            uid,
            user_ids,
            context=context)
    ...
    def action_account_confirmation_email(self, cr, uid, ids, context=None):
        """ create signup token for each user,
            and send their signup url by email
        """
        ...
        res_partner.signup_prepare(
            ...
            uid,
            partner_ids,
            signup_type="confirmation",
            expiration=self._now(days=+ 7),
            context=context)
