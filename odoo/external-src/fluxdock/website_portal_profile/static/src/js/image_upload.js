...
var slides = require('website_portal_profile.widgets');
var website = require('website.website');

var qweb = core.qweb;
var _t = core._t;

if(!$('.oe_image_js_upload').length) {
    return $.Deferred().reject("DOM doesn't contain '.oe_image_js_upload'");
}

ajax.loadXML('/website_portal_profile/static/src/xml/image_upload.xml', qweb);
...
var SlideDialog = Widget.extend({
    ...
    slide_upload: function (ev) {
        ...
        if (!(is_image || this.file.type === 'application/pdf')) {
            this.display_alert(_t("Invalid file type. Please select image file"));
            this.reset_file();
            return;
        }
    ...
    */
    ...
    $('.oe_image_js_upload').on('click', function () {
        var channel_id = $(this).attr('channel_id');
        slides.page_widgets['upload_image'] = new SlideDialog(this, channel_id).appendTo(document.body);
    });
